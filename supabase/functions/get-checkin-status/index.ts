...
Deno.serve(async (req): Promise<Response> => {
  ...
  try {
    ...
    const { data: dailyRewards } = await sb
      ...
      .select("day_number, reward_type, reward_amount, role_id, is_active")
      .order("day_number");

    const { data: bigReward } = await sb
      .from("checkin_big_reward")
      .select("reward_type, reward_amount, role_id, description, makeup_cost_per_day")
      .maybeSingle();
    ...
    return json({
      ...
      makeup_cost_per_day: bigReward?.makeup_cost_per_day ?? 50,

...
const BC_LIGHT = {
  ...
  honey:"#C4862A", honeyDim:"#A86820",
  cream:"#3D2008", creamDim:"#7A4A20",
  brownDark:"#3D2008",
  wall:"#C8A070", wallLight:"#D4A870", floor:"#B88850",
...
// ─── ITEMS — bear images ──────────────────────────────────────────────────────
const ITEMS = [
  { id:0, img:"/game1/bearG1.svg", name:"บักแว่น",    r:16, score:1,  glow:"rgba(139,94,60,0.55)"  },
  { id:1, img:"/game1/bearG2.svg", name:"บักอ้วน",    r:22, score:3,  glow:"rgba(196,134,42,0.5)"  },
  { id:2, img:"/game1/bearG3.svg", name:"บักอ้าซ่า",  r:28, score:7,  glow:"rgba(196,134,42,0.6)"  },
  { id:3, img:"/game1/bearG4.svg", name:"บักแพนด้า",  r:35, score:12, glow:"rgba(233,168,78,0.55)" },
  { id:4, img:"/game1/bearG5.svg", name:"บักโพล่า",   r:44, score:22, glow:"rgba(233,168,78,0.72)" },
  { id:5, img:"/game1/bearG6.svg", name:"บักสกินเฮด", r:54, score:55, glow:"rgba(233,168,78,0.92)" },
];
...
function drawScene(
  ...
  for (const b of balls) {
    ...
    if (b.type>=3) {
      ...
      // soft warm grounding shadow for small tiers — stays on-palette
      ctx.shadowColor="rgba(90,56,32,0.38)"; ctx.shadowBlur=5; ctx.shadowOffsetY=2;
    }
    ...
    <div style={{
      ...
      <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",maxWidth:canvasSize.w+8,marginBottom:8}}>
        ...
        <div style={{lineHeight:1.05,textAlign:"left",flex:1,minWidth:0,marginLeft:10}}>
          <div style={{fontSize:16,fontWeight:950,color:bc.textMain,letterSpacing:"-0.03em"}}>ร้านหมีชานมไข่มุก</div>
          <div style={{fontSize:10,fontWeight:800,color:bc.honey,letterSpacing:"0.08em",textTransform:"uppercase"}}>สะสมคะแนนรอไปก่อนนะ!</div>
        </div>
      ...
      <div ref={wrapRef} style={{
        ...
        {ascFlash&&(
          <div style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:18,display:"flex",alignItems:"center",justifyContent:"center",
            ...
            <img src="/game1/bearG6.svg" alt="Golden Bear" style={{width:96,height:96,filter:`drop-shadow(0 0 28px ${bc.honey})`,animation:"ascendBounce .6s ease-out",objectFit:"contain"}}/>
          </div>
        )}
      ...
      <div style={{display:"flex",alignItems:"stretch",gap:8,marginTop:10,width:"100%",maxWidth:canvasSize.w+8}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
          ...
          <span style={{fontSize:11,color:bc.honey,letterSpacing:"0.08em",fontWeight:900,marginBottom:4}}>ถ้วยต่อไป</span>
          <img src={ITEMS[nextId].img} alt={ITEMS[nextId].name} style={{width:36,height:36,objectFit:"contain"}}/>
          <span style={{fontSize:11,color:bc.textSub,marginTop:3}}>{ITEMS[nextId].name}</span>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:10,padding:"6px 14px",
        ...
        {ITEMS.map((item,i)=>(
          <span key={i} style={{display:"flex",alignItems:"center",gap:3}}>
            ...
            <span style={{fontSize:12,color:i===ITEMS.length-1?bc.honey:`${bc.honey}50`,fontWeight:i===ITEMS.length-1?700:400}}>
              {i===ITEMS.length-1?"↩":"→"}
            </span>
          </span>
        ))}
        <span style={{fontSize:12,color:bc.honey,fontWeight:900}}>วนกลับไข่มุก</span>
      </div>
    </div>
  );
}
...
function ScorePill({label,value,bc,gold,sm}:{label:string;value:number;bc:BC;gold?:boolean;sm?:boolean}) {
  return (
    <div style={{
      ...
      <span style={{fontSize:10,color:bc.honey,letterSpacing:"0.08em",fontWeight:700,textTransform:"uppercase"}}>{label}</span>
      <span style={{fontSize:sm?13:15,fontWeight:800,color:gold?bc.honey:bc.textMain,lineHeight:1.1}}>{value}</span>
    </div>
  );
}
